...
### 2026-09-20 — A rule that was only right because nothing had tested it
- What happened: Today's Log collapsed to "the most recent 4 hours", anchored to the **latest block's end**. For as long as the only blocks in a day were ones already logged, "latest block" and "most recent past block" were the same row, so the rule behaved exactly like "the last 4 hours of real time" and nobody could tell the two apart. Daily-defaults auto-blocks changed the shape of the data — now every day contains blocks sitting in the *future* — and the rule inverted: one 22:00 night routine pulled the window to 18:00–22:00, so at 11:00 the log showed tonight and hid the entire morning. The founder found it in use and reported it.
- Concept: **a rule can be correct by coincidence.** Two different definitions agree on the data you happen to have, so the wrong one passes every check you think to run — and keeps passing until a *different* feature changes the shape of the data underneath it. What broke here wasn't the windowing code (untouched for months) but daily defaults starting to write future-dated rows. Instance of PLAYBOOK Rule 4's silent-failure theme, and the same shape as the 2026-09-18 sleep-window bug one session earlier: there too, "starts between 02:00 and 10:00" had stood in for "is sleep" and only diverged once the founder configured a 09:00 morning routine. Two sessions running, the bug was an old proxy that a new feature falsified.
- In my words: (pending — asked at the 2026-09-23 wrap, founder chose no preference / skip. Per LEARNING_METHOD §4 a skip carries no penalty; ask again next wrap. The answer, for whoever asks it: both rules used an easy stand-in for the real thing, and a NEW feature — not a code change — made the stand-in false.)
