...
| `longPressAdd` | Long-Press + | **No** (just routes to existing entry points) | `renderAddControl`, `renderLongPressAddBtn`, `renderTodayAddControl` (alias), context map `_ADD_LONGPRESS` + `_addLongPress`, handlers `lpAddDown(e,context)` / `lpAddUp` / `lpAddCancel`, const `LONG_PRESS_MS`, state `_lpAddTimer` / `_lpAddFired` | `/* Long-Press + (experiment) */` | — | `renderAddControl` at the Today / Journal / Projects action rows — context-aware hold default per page |
