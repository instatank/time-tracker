...
| `weeklyReviews` | week-start date (**Sunday**) | `weekStart`, `weekEnd`, `totals{deepWorkHrs, dftRate, taskCompletionRate, adherencePct, winCount, dayRatingAvg}`, `starAverages{metricKey: avg}`, `patterns[]` (observation strings — from 2026-09-24 these are the Trends engine's sentences, e.g. "Best deep-work week in 9: 14h …"; older docs carry the retired `surfacePatterns` wording), `nextWeekIntention` (+ legacy `intention`), `tasks[{text, completed}]`, `aiSummary?`, `savedAt`. Unknown/added fields still render generically; `aiSummary` rendered as prose |
