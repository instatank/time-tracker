# Session handoff — reconciled 2026-07-06 (Phase C wrap); docs-only addenda 2026-07-07 + 2026-07-12 + 2026-07-16 + 2026-08-11; app-code addenda 2026-07-16 (2nd-brain alignment, SW v137) + 2026-08-15 (+ button unification, SW v138)
...
- **Branches:** `claude/*` branches auto-merge to `main` via GitHub Actions, `main` deploys to Vercel. (Check `git branch --show-current` — don't trust a doc for this.) Current working branch: `claude/plus-button-all-pages-kcriwx` (APP CODE this session — `+` button unification across Today/Journal/Projects; see the top addendum), pushed, no PR opened (auto-merge doesn't need one).
- **Service worker cache key:** check `const CACHE` at the top of `sw.js` — currently `dayos-v138` (bumped from v137 this session for the app-code change above). **Mandatory** to bump on every user-facing change — devices serve stale `index.html` otherwise. The `/ship` skill does this automatically.
- **Latest commit:** `6c201de` ("Replicate Today's + button... on Journal and Projects") on `claude/plus-button-all-pages-kcriwx` (check `git log`). Prior commits: `6570933` (docs-only, git-fetch instruction, 2026-08-11), `2d2077e` (last app-code wrap before this session, 2026-07-16).
