# Session handoff — reconciled 2026-07-06 (Phase C wrap); docs-only addenda 2026-07-07 + 2026-07-12 + 2026-07-16 + 2026-08-11 + 2026-08-16; app-code addenda 2026-07-16 (2nd-brain alignment, SW v137) + 2026-08-15 (+ button unification, SW v138)
...
- **Branches:** `claude/*` branches auto-merge to `main` via GitHub Actions, `main` deploys to Vercel. (Check `git branch --show-current` — don't trust a doc for this.) Current working branch: `claude/secrets-ci-workflows-zgok2r` (CI/config + docs, 2026-08-16 — secret scanning + merge gates; see the top addendum), pushed and already auto-merged to `main`, no PR opened (auto-merge doesn't need one). **New as of this session:** merges now wait on two required checks, `check.sh` and `gitleaks` — a red check blocks the auto-merge, which is a change from "every push ships unconditionally".
...
- **Latest commit:** `8084dc4` ("docs: record the repo-side secret guardrail in the security audit") on `claude/secrets-ci-workflows-zgok2r`, already on `main` (check `git log`). Prior commits: `601581f` (the CI/secret-scan gate itself, 2026-08-16), `43ff792` (the `docs/security-audit.md` write-up — copy graph, 7 verified-clean checks, 6 gaps, 4-phase plan; shipped 2026-08-15/16 and **not** recorded in this doc at the time, backfilled here), `6c201de` (last app-code change — `+` button unification, SW v138, 2026-08-15).
