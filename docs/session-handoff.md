# Session handoff — reconciled 2026-07-06 (Phase C wrap); docs-only addenda 2026-07-07 + 2026-07-12 + 2026-07-16 + 2026-08-11 + 2026-08-16 (×3); app-code addenda 2026-07-16 (2nd-brain alignment, SW v137) + 2026-08-15 (+ button unification, SW v138); server-only addendum 2026-08-16 (API hardening)
...
- **PENDING — founder action:** nothing in that commit changes production. `firebase deploy --only firestore:rules` is the only thing that makes these rules live. `playbook/SOP-deploy.md` now says so (new "Rules-change ritual" section + the surface-map row).
...
- **Branches:** `claude/*` branches auto-merge to `main` via GitHub Actions, `main` deploys to Vercel. (Check `git branch --show-current` — don't trust a doc for this.) Current working branch: `claude/firestore-rules-review-53vajs` (rules + config + docs, 2026-08-16 — `firestore.rules` into version control; see the top addendum), pushed. It also carries `0a52195` (server-only API hardening). Merges wait on two required checks, `check.sh` and `gitleaks` — a red check blocks the auto-merge, which is a change from "every push ships unconditionally".
- **Service worker cache key:** check `const CACHE` at the top of `sw.js` — currently `dayos-v138`, **unchanged on 2026-08-16** (that session touched only `.gitignore`, `.github/workflows/`, `.gitleaks.toml` and docs — no `index.html` change, so no bump was owed). It was last bumped v137 → v138 on 2026-08-15 for the `+` button work. **Mandatory** to bump on every user-facing change — devices serve stale `index.html` otherwise. The `/ship` skill does this automatically.
- **Latest commit:** `9e6ed72` ("Put firestore.rules under version control and review it") on `claude/firestore-rules-review-53vajs` (check `git log`). Prior commits: `0a52195` (AI-proxy + cron hardening, server-only, 2026-08-16), `fd2593a` (wrap for the detector-doctrine session), `8084dc4` + `601581f` (the CI/secret-scan gate, 2026-08-16), `43ff792` (the `docs/security-audit.md` write-up), `6c201de` (**last app-code change** — `+` button unification, SW v138, 2026-08-15). Three sessions in a row have now shipped no `index.html` change, which is why `dayos-v138` is still correct.
...
6. **`ALLOWED_UIDS` is not set in Vercel.** Until it is, the AI proxy still accepts any verified Firebase user — the allow-list code shipped, the config didn't.
7. **Neither rules file has been deployed from the repo.** `firebase deploy --only firestore:rules` / `--only storage`. Committing them changed nothing in production.
