# Session handoff — reconciled 2026-07-06 (Phase C wrap); docs-only addenda 2026-07-07 + 2026-07-12 + 2026-07-16
...
- **Branches:** `claude/*` branches auto-merge to `main` via GitHub Actions, `main` deploys to Vercel. (Check `git branch --show-current` — don't trust a doc for this.) Current working branch: `claude/dayos-timetracker-organization-stwbi6` (docs-only in this repo; the substantive work of the same session lives in `instatank/instatank42`), no PR opened yet (auto-merge doesn't need one).
- **Service worker cache key:** check `const CACHE` at the top of `sw.js` — currently `dayos-v136`, unchanged since 2026-07-06 (nothing user-facing shipped in this repo since). **Mandatory** to bump on every user-facing change — devices serve stale `index.html` otherwise. The `/ship` skill does this automatically.
- **Latest commit:** `f37cc78` — one-line pointer in `docs/second-brain-integration.md` to the agent's DayOS organization plan. Last app-code commit remains `0b46647` (Tag search Phase C).
