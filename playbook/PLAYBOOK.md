...
**EXECUTE:** 1) Read the repo's CLAUDE.md. 2) Read this playbook (locally if time-tracker is cloned, else via GitHub `get_file_contents` on `instatank/time-tracker`, `playbook/PLAYBOOK.md`). 3) Read the repo's handoff doc + `LEARNINGS.md` if present. 4) Before ending a session that shipped commits, run the repo's `/wrap` skill — the Stop hook will remind you once if you forget.
